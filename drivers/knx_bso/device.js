...
class KNXSunBlind extends KNXGenericDevice {
  ...
  onKNXEvent(groupaddress, data) {
    ...
    if (groupaddress === this.settings.ga_store_position) {
      this.setCapabilityValue('windowcoverings_set', DatapointTypeParser.percentage(data))
        .catch((knxerror) => {
          this.log('Set store position error', knxerror);
        });
    }
    if (groupaddress === this.settings.ga_slat_position) {
      this.setCapabilityValue('windowcoverings_tilt_set', DatapointTypeParser.percentage(data))
        .catch((knxerror) => {
  ...
  onCapabilitySunBlindPosition(value, opts) {
    if (this.knxInterface && this.settings.ga_store_position) {
      return this.knxInterface.writeKNXGroupAddress(this.settings.ga_store_position, 100 - value, 'DPT5')
        .catch((knxerror) => {
          this.log(knxerror);
  ...
  onCapabilitySlatSunBlindPosition(value, opts) {
    if (this.knxInterface && this.settings.ga_slat_position) {
      return this.knxInterface.writeKNXGroupAddress(this.settings.ga_slat_position, 100 - value, 'DPT5')
        .catch((knxerror) => {
          this.log(knxerror);
